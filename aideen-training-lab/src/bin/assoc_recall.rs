...
fn main() {
    ...
    #[cfg(feature = "wgpu")]
    {
        ...
        if trainer.gpu_deq.is_none() {
            eprintln!("ERROR: assoc_recall requires an active GPU backend; the current run fell back to an invalid path.");
            std::process::exit(1);
        }
    }
            ...
            if let Some(gpu) = trainer.gpu_deq.as_ref() {
                ...
                for s in 0..trainer.config.h_slots {
                    ...
                    println!(
                        ...
                        s,
                        fw[diag_base],       // +0 avg read_entropy  (↓ = specialization)
                        fw[diag_base + 1],   // +1 max read_max_prob (↑ = relies on 1 bank)
                        ...
                        fw[diag_base + 8],   // +8 r_n (read counter)
                        fw[diag_base + 9],   // +9 w_n (write counter)
                    );
                }
            }
            flush();
