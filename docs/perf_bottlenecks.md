...
### 2026-03-26 — Provisional baseline (real TPS, batch=4, no model changes)
**Context**: real throughput on M1 Pro, full GPU training, without touching the math.
**Goal**: establish a fixed baseline to measure improvements.
...
- Dataset: `tinyshakespeare.txt` (token cache reused)
- Profile: real training (LM + DEQ + adjoint)
- Code: `e88dcb5`
...
- If the same config on another commit produces TPS < 70 or a divergent loss, revalidate.
- If `aideen-training-lab/src/trainer.rs` or `aideen-backbone/src/shaders/lm_train.wgsl` is modified, repeat this baseline.

**Scope**
- Valid for Apple M1 Pro, batch=4, ctx=256, h_slots=8, d=512.

---

## Benchmark Profiles (fixed usage)

Do not use a single configuration for everything. In AIDEEN there are three distinct questions:
- whether a change breaks or alters stability,
- whether it improves hot-path throughput,
- or whether it lowers the total system overhead.

Each one requires a distinct profile. Do not compare numbers across profiles.

### Profile A — Validation
**Goal**: validate correctness/stability when a change touches backward, history, adjoint, DEQ or numerics.
...
**Use**
- kernel fusions in solver/backward
- changes in the staged adjoint
- changes in history
- new mathematically-equivalent paths that have not yet been validated

### Profile B — Regime
**Goal**: measure real throughput of the repetitive hot path. This is the main metric for structural performance.
...
**Use**
- only after passing Profile A if the change touches the math
- shader/dispatch/buffer-lifecycle optimizations aimed at raising regime TPS

**Provisional reference**
- chunk 10: loss=5.1464, tps=81.3
- chunk 20: loss=5.6460, tps=83.3

### Profile C — Total System
**Goal**: measure total overhead of the short run. Useful for startup, readbacks, bind groups, observability and costs outside the hot path.
...
**Use**
- driver/setup costs
- time lost to readbacks
- bind group churn
- improvements that do not necessarily move the regime but do move the total system time

### Usage rule
- If a change touches the math or solver: `Profile A` -> `Profile B`
- If a change touches only overhead: `Profile C`, and then `Profile B` if it looks promising
- Do not decide on the TPS of one profile using numbers from another profile
- Do not promote defaults just because they "compile and do not crash"; they must pass the corresponding profile

### 2026-03-31 — Fused path stabilized, observability moved off hot path
**Context**: fused trainer with stable history, without `fixed-history` contaminating training, and with readbacks/progress/validation off the critical path by default.

**Structural changes**
- `fixed-history` remains only as a semantic reference for `eval/generate`
- `train_on_tokens` / `train_on_file` no longer force loss readbacks inside the fused step
- progress and validation do not block by default (`VAL_EVERY=0`, `PROGRESS_EVERY=0`)
- the `ctx_len` default of the train binary rises to `512`

**Recent results**
- stable profile, `batch=4`, `ctx=512`, `B19=1`, history default:
  - `tps_epoch = 4123.0`
- stable profile, `batch=8`, `ctx=512`, `B19=1`, history default:
  - `tps_epoch = 5197.8`
- comparative ceiling, `batch=8`, `ctx=512`, `B19=1`, `AIDEEN_DEQ_HIST_GATED=0`:
  - `tps_epoch = 5129.0`

**Reading**
- the collapse to `~800 TPS` was not a model limit; it came from mixing profiles and from the token-by-token reference lane
- the fused hot path has returned to a healthy and scalable band
...
**Revalidation (2026-03-31, real fused train, tinyshakespeare, batch=8, ctx=512, B19=1)**:
- `AIDEEN_ADJ_ITERS_OVERRIDE=4` gave `tps_epoch = 2225.6`
- baseline of the same profile without the override: `tps_epoch = 5197.8`

Current conclusion:
- in the real fused training regime, lowering `adj_iters` to `4` is **not** a safe improvement
- it is ruled out as a default for now
