...
pub fn lmhead_backward(
    ...
    let dw = dl_dlogits * h_pooled.transpose();
    let db = dl_dlogits.clone();
    let dl_dh = w_lm.transpose() * dl_dlogits;
    (LmHeadGrads { dw, db }, dl_dh)
}
...
pub fn deq_implicit_grad<R: Reasoning>(
    ...
    DVector::from_iterator(d_r, v.iter().take(d_r).copied())
}
...
mod tests {
    ...
    fn lmhead_grad_has_correct_shapes() {
        ...
        let (grads, dl_dh) = lmhead_backward(&dl, &h, &w);
    ...
    fn lmhead_grad_reduces_loss() {
        ...
        let (grads, _) = lmhead_backward(&dl, &h_pooled, &head.w);
