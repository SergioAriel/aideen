...
//!
//! Uso:
//!   cargo run --release --features wgpu -p aideen-backbone --bin benchmark
...
fn run_training_benchmark(use_gpu: bool, title: &str) {
    ...
    if warmup_epochs > 0 {
        ...
        for w in 0..warmup_epochs {
            let tw = Instant::now();
            trainer.train_sequence(train_tokens, targets);
            println!(
                "    warmup {} listo en {}ms",
                w + 1,
    ...
    for epoch in 0..epochs {
        ...
        for i in (0..train_tokens.len().saturating_sub(ctx_len)).step_by(step) {
            ...
            epoch_loss += trainer.train_sequence(window_tokens, window_targets);
            num_steps += 1;
        }

        // Si la secuencia es muy corta y no entró en el loop, o para el resto final:
        if num_steps == 0 && train_tokens.len() >= 2 {
            epoch_loss = trainer.train_sequence(train_tokens, targets);
            num_steps = 1;
        }
