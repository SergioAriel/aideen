...
/// FixedPointMemoryReasoning — the actual `f` block of the DEQ.
pub struct FixedPointMemoryReasoning {
    ...
    // ── Fixed-Point Memory SSM per slot ───────────────────────────────────────────────────
    #[cfg(feature = "lab")]
...
impl FixedPointMemoryReasoning {
    ...
    fn new_with_rng<R: Rng + ?Sized>(config: ArchitectureConfig, rng: &mut R) -> Self {
        ...
        // The composed attention Jacobian (W_o · softmax · W_v · W_q) can exceed 1
        ...
        // residual_alpha=0.0 is necessary — even alpha=0.2 drives contr→1.
    ...
    pub fn renormalize_weights(&mut self) {
        // Threshold 0.10 for attention matrices — needed to keep σ(J_attn) < 1
        // during training (not only at initialization).
        // w_x and w_out (external Fixed-Point Memory) use threshold 0.70 — they do not affect DEQ contractivity.
        let slot_coord_mode = DeqRuntimeConfig::from_env().has_explicit_slot_comparison();
        let attn_t = if slot_coord_mode { 0.30_f32 } else { 0.10_f32 };
        let win_t = 0.30_f32;
        let fpm_t = 0.70_f32;
...
mod tests {
    ...
    fn step_is_sensitive_to_input() {
        ...
        assert_ne!(
            h_a.to_flat(),
            h_b.to_flat(),
            "step must be sensitive to the input s"
        );
    }
    ...
    fn step_is_stable_no_explosion() {
        ...
        assert!(energy.is_finite(), "Energy must not blow up: {energy}");
    ...
    fn fixed_hist_ctx_is_zero_without_memory() {
        ...
        assert!(
            ...
            "Without m_prev the hist_ctx must be exactly zero"
        );
    }
    ...
    fn zero_fixed_hist_ctx_matches_plain_step() {
        ...
        for (a, b) in plain.to_flat().iter().zip(with_hist.to_flat().iter()) {
            assert!(
                (a - b).abs() < 1e-6,
                "step_with_fixed_hist_ctx(0) must match step: {a} vs {b}"
            );
        }
    }
}
