...
/// Result of a generation, for comparison across strategies.
#[derive(Debug, Clone)]
...
impl GenerationResult {
    /// Diversity: proportion of unique tokens (0=all equal, 1=all distinct).
...
/// No decoder, no autoregression — at most K tokens.
///
...
/// N-layer Fixed-Point Memory decoder conditioned on H* via FiLM.
/// Autoregressive at the token level. Arbitrary length.
...
/// over query + previous tokens.
///
/// Cost: max_deq_iters × number_of_tokens_to_generate (expensive without GPU).
...
impl DeqAutoRegStrategy {
        ...
        R: Reasoning,
    {
        ...
        for _ in 0..max_tokens {
            // ① Run the DEQ over the current query (with generated tokens embedded)
            ...
            // ② Read the token from the resulting H*
            ...
            // ③ Update the query with the generated token
            // We use the real size of current_query (D_R + D_SIM) to avoid a mismatch
...
/// Returns the results sorted by token diversity (descending).
...
mod tests {
    ...
    fn slot_direct_sensitive_to_h_star() {
        ...
        assert_ne!(
            ...
            "SlotDirect must differ for distinct H*"
        );
    }
    ...
    fn decoder_sensitive_to_h_star() {
        ...
        assert_ne!(
            r_a.tokens, r_b.tokens,
            "Decoder must differ for distinct H*"
        );
    }
    ...
    fn deq_autoreg_sensitive_to_query() {
        ...
        assert_ne!(
            r_a.tokens, r_b.tokens,
            "DeqAutoReg must differ for distinct queries"
        );
    }
