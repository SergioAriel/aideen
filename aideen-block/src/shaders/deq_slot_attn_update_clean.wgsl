...
fn deq_slot_attn_update_main(
    ...
    let h_base_t = (batch_idx * shape.seq_len + shape.token_start) * total_elements;
    let signal_span = d_model * h_slots;
    let scratch_stride = signal_span * 5u;
    let batch_scratch_t = (batch_idx * shape.seq_len + shape.token_start) * scratch_stride;
    let signal_base = batch_scratch_t + slot_off;
    let q_base = batch_scratch_t + signal_span + slot_off;
    let k_root = batch_scratch_t + signal_span * 2u;
    let v_root = batch_scratch_t + signal_span * 3u;
    ...
    if (tid < h_slots) {
        let ks = tid;
        let k_off = k_root + ks * d_model;
        var score = 0.0;
        for (var d = 0u; d < head_dim; d = d + 1u) {
            score = score + Scratch[q_base + d] * Scratch[k_off + d];
        }
    ...
    for (var h = tid; h < head_dim; h = h + WG_SIZE) {
        var mix = 0.0;
        for (var ks = 0u; ks < h_slots; ks = ks + 1u) {
            mix = mix + slot_attn_weights[ks] * Scratch[v_root + ks * d_model + h];
        }
        head_mix[h] = mix;
    }
    workgroupBarrier();
    ...
    if (tid == 0u) {
        ...
        DebugLog[slot_base + 1u] = select(1.0, 0.0, converged);
        DebugLog[slot_base + 2u] = f32(total_iters_seen);
