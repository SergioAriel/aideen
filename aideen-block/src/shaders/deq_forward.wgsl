...
fn deq_forward_main(
    ...
    for (var t = 0u; t < shape.token_count; t = t + 1u) {
        ...
        // Experiment phase 1+2: H_curr remains the iterative state of the current token solve,
        // but it no longer carries across tokens. History now enters through hist_ctx.
        var local_sumsq = 0.0;
        for (var d = tid; d < d_model; d = d + WG_SIZE) {
